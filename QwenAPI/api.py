...
def return_key_word(sentence):
    message="请从以下描述中提取关键词，确保这些词汇能够精准反映查询意图：- 提取名词、形容词和动词，这些词汇应直接与查询内容相关。可能会有打错的词，自行改正" \
            f"- 避免提取过于通用的或不太可能作为查询关键词的词（如“一个”、“我想”等），如果有介词（例如在，at，in等）后面跟了名词，将名词作为一个关键词，一定是一个词而不是短语。" \
            f"- 如果有名词动词在句中相邻且连起来意思通顺，联想词只能是一个词，名词或者动词" \
            f"- 可以对加上对句子中提取出来的关键词进行合理联想，但都要是意思相近或者同一方面的词（例如输入ai可以联想到大模型，计算机视觉等），只用联想三四个就行。如果关键词是英文，联想词也要是英文" \
            f"将联想词和关键词都返回,如果是英文则都要转小写。用户输入：{sentence}返回格式：提取关键词：[关键词1,关键词2,关键词3] 联想关键词：[关键词1,关键词2]"\
    ...
    return extraction_keywords,association_keywords
